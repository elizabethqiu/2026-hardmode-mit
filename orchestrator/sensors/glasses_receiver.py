...
class GlassesReceiver:
    ...
    Holds recent data and pending TTS for the orchestrator to consume.
    """
    ...
    def set_on_data(self, callback: Callable[[dict], None]):
        """Set callback invoked when new glasses data arrives."""
        self._on_data = callback

    def get_snapshot(self) -> dict:
        """Return current glasses state and clear transient fields."""
        with self._lock:
            out = {
    ...
    def push_tts(self, text: str):
        """Add TTS for the MiniApp to poll and speak."""
        with self._lock:
            self._pending_tts.append({"text": text})

    def pop_tts(self) -> dict | None:
        """Pop next pending TTS for MiniApp. Returns None if empty."""
        with self._lock:
