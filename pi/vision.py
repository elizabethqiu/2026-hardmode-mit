"""
vision.py — MediaPipe face mesh + eye/gaze tracking via Logitech Brio 101

Streams dicts of the form:
  {
    "eye_aspect_ratio": float,   # 0=closed, 1=wide open
    "gaze_score":       float,   # 0=looking away, 1=looking at screen
    "face_detected":    bool,
  }

Run in a daemon thread; yields ~10fps results.
"""
...
log = logging.getLogger("enoki.vision")

# MediaPipe landmark indices for eye corners and eyelids
# Left eye: 33(outer), 160, 158, 133(inner), 153, 144
# Right eye: 362(outer), 385, 387, 263(inner), 373, 380
LEFT_EYE_IDX  = [33, 160, 158, 133, 153, 144]
RIGHT_EYE_IDX = [362, 385, 387, 263, 373, 380]

# Iris indices (MediaPipe face mesh with refine_landmarks=True)
LEFT_IRIS_IDX  = [474, 475, 476, 477]
RIGHT_IRIS_IDX = [469, 470, 471, 472]


def _eye_aspect_ratio(landmarks, indices, image_w, image_h):
    """EAR = vertical / horizontal eye openness. Low EAR = closing/closed."""
    pts = np.array([
        [landmarks[i].x * image_w, landmarks[i].y * image_h]
        for i in indices
    ])
    # Vertical: avg of two vertical pairs
...
def _gaze_score(landmarks, left_iris, right_iris, image_w, image_h):
    """
    Rough gaze: compare iris center to eye bounding box center.
    1.0 = iris centered (looking at screen), 0.0 = iris far off-center.
    """
    ...
    offset = (iris_offset(LEFT_EYE_IDX, left_iris) +
               iris_offset(RIGHT_EYE_IDX, right_iris)) / 2.0
    return max(0.0, 1.0 - offset * 4.0)   # scale: offset 0.25 → score 0


class VisionProcessor:
    def __init__(self, camera_index: int = 0, fps_target: int = 10):
